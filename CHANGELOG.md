...
- **需求**：用两个排行榜替换原有热力图
- **修改**：
  - 排行榜一：🏆 学生发言榜，按次数降序，🥇🥈🥉 奖牌 + 进度条
  - 排行榜二：👥 小组发言榜，按组汇总总次数降序
  - `speak_api.php`：新增 `getGroupRanking` 接口
  - `speak.php`：右栏 HTML 替换 + JS 全部重写
  - +1/-1/清空 操作后同步刷新两个排行榜
- **文件**：`pages/teacher/speak.php`, `pages/teacher/speak_api.php`
- **状态**：✅ 已完成
